...
import middlewares
import pytest
from tests.types import MockPayload, InboxType, OutboxType
...
async def test_not_called_call_next():
    ...
    with pytest.raises(middlewares.CallNextNotUsedError) as error:
        read_afterwords = await middlewares.read_forewords(middleware1, middleware2, inbox_value=inbox_value)
...
async def test_not_returned():
    ...
    async def middleware2(inbox: InboxType, call_next: middlewares.types.CallNext) -> typing.Optional[OutboxType]:
        middleware2_before_call(MockPayload(inbox, time.monotonic()))
        outbox = await call_next()
