...
import middlewares
from tests.types import InboxType, OutboxType, MockPayload
...
async def test_normal_case():
    ...
    async def middleware1(inbox: InboxType, call_next: middlewares.types.CallNext) -> OutboxType:
    ...
    async def middleware2(inbox: InboxType, call_next: middlewares.types.CallNext) -> OutboxType:
    ...
    middleware1_run_info: MockPayload = middleware1_before_call.call_args[0][0]
    middleware2_run_info: MockPayload = middleware2_before_call.call_args[0][0]

    # Middleware2 called later
    assert middleware1_run_info.stamp < middleware2_run_info.stamp

    # Inbox value has been passed
    assert middleware1_run_info.value == middleware2_run_info.value == inbox_value
    ...
    assert middleware1_run_info.value == middleware2_run_info.value == outbox_value
